...
fn decodeEntity(allocator: std.mem.Allocator, entity: []const u8) ![]const u8 {
    ...
    if (entity[0] == '#') {
        if (entity.len == 1) return error.InvalidEntity;

        const num = if (entity.len >= 2 and (entity[1] == 'x' or entity[1] == 'X'))
            std.fmt.parseInt(u21, entity[2..], 16) catch return error.InvalidEntity
        else
            std.fmt.parseInt(u21, entity[1..], 10) catch return error.InvalidEntity;

        if (num == 0) {
            return allocator.dupe(u8, &[_]u8{ 0xEF, 0xBF, 0xBD });
        }
        const code: u21 = @intCast(num);
        var buf: [4]u8 = undefined;
        const len = std.unicode.utf8Encode(code, &buf) catch return error.InvalidEntity;
    ...
    var with_amp = try std.ArrayList(u8).initCapacity(allocator, entity.len + 2);
    try with_amp.append(allocator, '&');
    try with_amp.appendSlice(allocator, entity);
    try with_amp.append(allocator, ';');
    return with_amp.toOwnedSlice(allocator);
}
...
pub fn decodeEntities(allocator: std.mem.Allocator, text: []const u8) ![]const u8 {
    ...
    var pos: usize = 0;
    while (pos < text.len) {
        if (pos + 1 < text.len and text[pos] == '&' and !isEscaped(text, pos)) {
            var end = pos + 1;
            while (end < text.len and text[end] != ';') : (end += 1) {
                // NOTE: There are no entities with names longer than 31 chars
                if (end - pos > 31) break;
            }
            if (end < text.len and text[end] == ';') {
                const entity = text[pos + 1 .. end];
                const decoded = decodeEntity(allocator, entity) catch {
                    // If decoding fails, keep the original entity as-is
                    const original = try std.fmt.allocPrint(allocator, "&{s};", .{entity});
