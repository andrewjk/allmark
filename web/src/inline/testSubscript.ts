...
function testSubscript(state: InlineParserState, parent: MarkdownNode, _end: number): boolean {
	...
	if (char === "~" && !isEscaped(state.src, state.i)) {
		...
		if (markup.length < 3) {
			...
			if (rightFlanking) {
				...
				// Check if it's a closing subscript
				if (startDelimiter !== undefined) {
					// Convert the text node into a subscript node with a new
			...
			if (leftFlanking) {
				// Check if it's an opening subscript
				// Add a new text node which may turn into subscript
