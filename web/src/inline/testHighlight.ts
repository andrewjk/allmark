...
function testHighlight(state: InlineParserState, parent: MarkdownNode, _end: number): boolean {
	...
	if (char === "=" && !isEscaped(state.src, state.i)) {
		...
		if (markup.length < 3) {
			...
			if (rightFlanking) {
				...
				// Check if it's a closing highlight
				if (startDelimiter !== undefined) {
					// Convert the text node into a highlight node with a new
			...
			if (leftFlanking) {
				// Check if it's an opening highlight
				// Add a new text node which may turn into highlight
