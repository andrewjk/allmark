	...
	if (char === "^" && !isEscaped(state.src, state.i)) {
		...
		if (markup.length < 3) {
			...
			if (rightFlanking) {
				...
				// Check if it's a closing superscript
				if (startDelimiter !== undefined) {
					// Convert the text node into a superscript node with a new
			...
			if (leftFlanking) {
				// Check if it's an opening superscript
				// Add a new text node which may turn into superscript
